...
QUnit.test("getAsync on a HasOne relation", function () {
  ...
  var request = person.getAsync('user', {
    error: function () {
      errorCount++;
    }
  });
...
QUnit.test("getAsync on a HasMany relation", function () {
  ...
  var request = zoo.getAsync('animals', {
    error: function () {
      errorCount++;
    }
  });
...
QUnit.test("`toJSON` cascades for non-relation Backbone.Model attributes", function () {
  ...
  var RawHolder = Backbone.Relational.Model.extend({});

  var model = new RawHolder({
    id: 'raw-1',
    rawChild: new Child()
  });

  var json = model.toJSON();
  ok(_.isObject(json.rawChild), 'rawChild serialized to object');
  equal(json.rawChild.id, '1', 'rawChild.id present');
  equal(json.rawChild.name, 'First node', 'rawChild.name present');
});

QUnit.test("`toJSON` respects includeInJSON variants (HasOne) and still cascades for non-relations", function () {
  var scope = {};
  Backbone.Relational.store.addModelScope(scope);

  scope.Child = Backbone.Relational.Model.extend({
    idAttribute: 'id',
    defaults: function () {
      return {id: 'c-1', name: 'Kid', age: 7};
    }
  });

  var HolderAll = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasOne, key: 'child', relatedModel: 'Child', includeInJSON: true}]
  });
  var HolderId = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasOne, key: 'child', relatedModel: 'Child', includeInJSON: 'id', keyDestination: 'child_id'}]
  });
  var HolderSome = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasOne, key: 'child', relatedModel: 'Child', includeInJSON: ['name']}]
  });
  var HolderNone = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasOne, key: 'child', relatedModel: 'Child', includeInJSON: false}]
  });

  var c = new scope.Child({id: 'c-42', name: 'Jane', age: 9});

  var all = new HolderAll({id: 'h1', child: c, raw: new scope.Child({id: 'raw-1', name: 'Raw'})});
  var jAll = all.toJSON();
  ok(_.isObject(jAll.child), 'includeInJSON:true outputs object');
  equal(jAll.child.id, 'c-42', 'relation child has id');
  equal(jAll.child.name, 'Jane', 'relation child has name');
  ok(_.isObject(jAll.raw), 'non-relation attribute cascades');
  equal(jAll.raw.id, 'raw-1', 'raw non-relation has id');
  equal(jAll.raw.name, 'Raw', 'raw non-relation has name');

  var onlyId = new HolderId({id: 'h2', child: c});
  var jId = onlyId.toJSON();
  equal(jId.child_id, 'c-42', 'includeInJSON:"id" outputs id at keyDestination');
  ok(!jId.child, 'original relation key removed (keyDestination in use)');

  var someHasOne = new HolderSome({id: 'h3', child: c});
  var jSome = someHasOne.toJSON();
  ok(_.isObject(jSome.child), 'includeInJSON:[keys] outputs object');
  equal(jSome.child.name, 'Jane', 'selected attribute present');
  ok(!('age' in jSome.child), 'non-selected attribute omitted');

  var none = new HolderNone({id: 'h4', child: c});
  var jNone = none.toJSON();
  ok(jNone.child === undefined, 'includeInJSON:false omits relation from JSON');
});

QUnit.test("`toJSON` respects includeInJSON variants (HasMany) and does not double-process", function () {
  var scope = {};
  Backbone.Relational.store.addModelScope(scope);
  scope.Item = Backbone.Relational.Model.extend({idAttribute: 'id', defaults: {name: ''}});
  var ManyAll = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasMany, key: 'items', relatedModel: 'Item', includeInJSON: true}]
  });
  var ManyIds = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasMany, key: 'items', relatedModel: 'Item', includeInJSON: 'id'}]
  });
  var ManySome = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasMany, key: 'items', relatedModel: 'Item', includeInJSON: ['id', 'name']}]
  });
  var ManyNone = Backbone.Relational.Model.extend({
    relations: [{type: Backbone.Relational.HasMany, key: 'items', relatedModel: 'Item', includeInJSON: false}]
  });

  var i1 = new scope.Item({id: 'i1', name: 'One'});
  var i2 = new scope.Item({id: 'i2', name: 'Two'});

  var all = new ManyAll({id: 'm1', items: [i1, i2]});
  var jAll = all.toJSON();
  ok(_.isArray(jAll.items), 'HasMany includeInJSON:true yields array');
  ok(_.isObject(jAll.items[0]), 'items are objects');

  var ids = new ManyIds({id: 'm2', items: [i1, i2]});
  var jIds = ids.toJSON();
  ok(_.isArray(jIds.items), 'HasMany includeInJSON:"id" yields array');
  equal(jIds.items.length, 2, 'HasMany includeInJSON:"id" yields two ids');
  ok(jIds.items.indexOf('i1') !== -1 && jIds.items.indexOf('i2') !== -1, 'ids array contains i1 and i2');

  var someMany = new ManySome({id: 'm3', items: [i1, i2]});
  var jSome = someMany.toJSON();
  ok(jSome.items[0].id && jSome.items[0].name, 'selected attributes included');
  ok(!('other' in jSome.items[0]), 'non-selected attributes omitted');

  var none = new ManyNone({id: 'm4', items: [i1, i2]});
  var jNone = none.toJSON();
  ok(jNone.items === undefined, 'includeInJSON:false omits HasMany relation');
});
