...
QUnit.test('getRelations', function () {
	...
	ok(
		_.every(relations, function (rel) {
...
QUnit.test('getAsync on a HasMany relation', function () {
	...
	equal(
		...
		1,
		'An error occured when fetching, but no DELETE requests are made to the server while handling local cleanup.'
	);
...
QUnit.test(
	"'toJSON' should return null for relations that are set to null, even when model is not fetched",
	function () {
...
QUnit.test(
	"`toJSON` should include ids for 'unknown' or 'missing' models (if `includeInJSON` is `idAttribute`)",
	function () {
		// See GH-191
...
QUnit.test(
	'By default, `parse` should only get called on top-level objects; not for nested models and collections',
	function () {
		var companyData = {
			data: {
				id: 'company-1',
...
QUnit.test(
	"Initialization and sharing of 'superModel' reverse relations by adding to a polymorphic HasMany",
	function () {
		var scope = {};
		Backbone.Relational.store.addModelScope(scope);
		scope.PetAnimal = Backbone.Relational.Model.extend({
