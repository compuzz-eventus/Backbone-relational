...
QUnit.test('triggers appropriate add / remove / change events on bulk assignment', function () {
	...
	house.get('occupants').on('change:nickname', function (model) {
		ok(
			model.id === 'person-8',
			"Only person-8 should have it's nickname updated: " + model.id + ' nickname updated'
		);
		changeEventsTriggered++;
	});
	...
	ok(
		removeEventsTriggered === 1,
		'Exactly one remove event was triggered (triggered ' + removeEventsTriggered + ' events)'
	);
	ok(
		changeEventsTriggered === 1,
		'Exactly one change event was triggered (triggered ' + changeEventsTriggered + ' events)'
	);
});

QUnit.test(
	'triggers appropriate change events even when callbacks have triggered set with an unchanging value',
	function () {
		var house = new House({
			id: 'house-100',
			location: 'in the middle of the street'
