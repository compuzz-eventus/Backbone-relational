...
QUnit.test("Listeners for 'add'/'remove', on a HasMany relation, for a Model with multiple relations", function () {
	...
	oldCompany.on('add:employees', function (model, coll) {
		...
		ok(
			model.get('company') instanceof Company && model.get('person') instanceof Person,
	...
	person1.on('add:jobs', function (model, coll) {
		ok(
			model.get('company') === oldCompany && model.get('person') === person1,
			'Both Person and Company are set on the Job instance'
		);
	});
...
QUnit.test(
	'Raw-models set to a hasMany relation do trigger an add event in the underlying Collection with a correct index',
	function () {
		var zoo = new Zoo();
...
QUnit.test(
	'Models set to a hasMany relation do trigger an add event in the underlying Collection with a correct index',
	function () {
		var zoo = new Zoo();
...
QUnit.test(
	"The 'collectionKey' options is used to create references on generated Collections back to its RelationalModel",
	function () {
		var zoo = new Zoo({
