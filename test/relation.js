...
QUnit.test('`includeInJSON` (Person to JSON)', function () {
	...
	equal(
		json.user_id,
		'user-1',
		"The value of 'user_id' is the user's id (not an object, since 'includeInJSON' is set to the idAttribute)"
	);
...
QUnit.test("'keyDestination' saves to 'key'", function () {
	...
	ok(
		viewJSON.properties_attributes && viewJSON.properties_attributes.length === 2,
		"'viewJSON' has two 'properties_attributes'"
	);
...
QUnit.test('Repeated model initialization and a collection should not break existing models', function () {
	...
	ok(
		companyA.get('employees') instanceof Backbone.Relational.Collection,
		"Company's employees should be a collection"
	);
	...
	ok(
		companyA.get('employees') instanceof Backbone.Relational.Collection,
		"Company's employees should still be a collection"
	);
	equal(companyA.get('employees').length, 2, 'with elements');
});
