...
QUnit.test('Models are created from objects, can then be found, destroyed, cannot be found anymore', function () {
	...
	ok(
		anotherHouse.get('occupants').get(personId) instanceof Person,
		"Occupants contains the Person with id='" + personId + "'"
	);
	...
	ok(
		!anotherHouse.get('occupants').get(personId),
		"Occupants no longer contains the Person with id='" + personId + "'"
	);
...
QUnit.test(
	'Model.collection is the first collection a Model is added to by an end-user (not its Backbone.Relational.Store collection!)',
	function () {
...
QUnit.test("Inheritance with `subModelTypes` uses the same collection as the model's super", function () {
	...
	equal(
		Backbone.Relational.store._collections.length,
		numCollections,
		'`_collections` should have remained the same'
	);
